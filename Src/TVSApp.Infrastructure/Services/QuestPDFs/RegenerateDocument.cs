...
public class RegenerateDocument : QuestPdf
{
    ...
    public override void Compose(IDocumentContainer document)
    {
            ...
            document.Page(page =>
            {
                ...
                {   
                    ...
                    column.Item().Element(container =>
                    {
                        ...
                        {
                            row.Spacing(0);
                            row.ConstantItem(400).Text($"{_serviceOrder.Id:00'.'000}")
                                .FontSize(120)
                                .ExtraBold()
        ...
        else if (_serviceOrder.DeliveryDate.HasValue && _serviceOrder.Product.Defect != "VENDA")
        {
            document.Page(page =>
            {
                page.Margin(8);
        ...
        else if (_serviceOrder.DeliveryDate.HasValue && _serviceOrder.Product.Defect == "VENDA")
        {
            document.Page(page =>
            {
                ...
                page.Content().Column(column =>
                {
                    ...
                    column.Item().PaddingVertical(10).Element(container =>
                    {
                        container.Border(1).BorderColor(Colors.Grey.Lighten1).Background(Colors.Grey.Lighten4).Padding(10).Row(row =>
                        {
                            ...
                            row.RelativeItem().Column(col =>
                            {
                                ...
                                col.Item().Row(r => 
                                {
                                    r.ConstantItem(80).Text("VENDA DE: ").FontSize(11).SemiBold().FontColor(Colors.Grey.Darken2);
                                    r.RelativeItem().Text(_serviceOrder.Product.Type.GetDisplayName())
                                        .FontSize(11).Bold().FontColor(Colors.Black);
                                });
                                ...
                                col.Item().Row(r => 
                                {
                                    r.ConstantItem(80).Text("DATA:").FontSize(11).SemiBold().FontColor(Colors.Grey.Darken2);
                                    r.RelativeItem().Text(_serviceOrder.DeliveryDate?.ToString("dd/MM/yyyy") ?? "-")
                                        .FontSize(11).Bold().FontColor(Colors.Black);
                                });
                            });
                    ...
                    column.Item().PaddingVertical(20).LineHorizontal(1).LineColor(Colors.Red.Lighten1);
                });
            });
        }
    }
